...
def load_json_argv( sysargv ):
    ...
    if len(sysargv)!=2:
        printerr (f"Error in argument, should add a single .json questionnaire. Example : {ntpath.basename(sys.argv[0])} questionnaire_to_read.json")
        return None
    ...
    if validators.url(filename) :    # si le fichier JSON est une URL
        try:
            ...
            printerr(f"Invalid URL at : {filename}")
            return None
        else:
            if response.status_code == 404:
                printerr(f"File not found at : {filename}")
                return None
            else:
                try:
